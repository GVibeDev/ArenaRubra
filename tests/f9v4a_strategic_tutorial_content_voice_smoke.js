...
// F9V4a strategic dialogue and faction voice pass.
let dialogueAudit = ev("tutorialRuntimeDialogueAuditF9V4a()");
if (!dialogueAudit.ok || dialogueAudit.scenarios !== 5 || dialogueAudit.voices !== 5 || dialogueAudit.topics < 10) throw new Error(`Dialogue audit failed ${JSON.stringify(dialogueAudit)}`);

const topicChecks = [
  ["lesson-2-nexus","read-starter-structure",/Struttura può occupare un PS/],
  ["lesson-2-nexus","deployment-network-online",/nello stesso turno/],
  ["lesson-1-exordium","explain-defense",/ATT eccedente non passa agli HP/],
  ["lesson-1-exordium","mech-arrival",/Concentra tattica e più unità/],
  ["lesson-1-exordium","lesson-complete",/Depot ENE.*ricarica.*tattica|Depot ENE.*tattica.*ricarica/],
  ["lesson-3-agathoi","agathoi-lesson-complete",/Non puoi difendere ogni PS/],
  ["lesson-4-liberti","liberti-lesson-complete",/PS più difeso.*quello sguarnito/],
  ["lesson-4-liberti","capture-center-ps",/rioccupi il Punto/],
  ["lesson-5-fabeot","fabeot-lesson-complete",/motore della partita ruota sui PS/],
  ["lesson-5-fabeot","fabeot-lesson-complete",/troppo protetto.*minacciane un altro/]
];
for (const [scenarioId, stepId, pattern] of topicChecks) {
  const text = ev(`tutorialRuntimeDialoguePatchF9V4a(${JSON.stringify(scenarioId)},${JSON.stringify(stepId)}).text`);
  if (!pattern.test(text)) throw new Error(`Strategic topic missing ${scenarioId}/${stepId}: ${text}`);
}

for (const [faction, tone] of Object.entries({Exordium:"militaresco_audace",Nexus:"neutro_robotico",Agathoi:"cauto_sospettoso",Liberti:"selvaggio_aggressivo",Fabeot:"aristocratico_subdolo"})) {
  const actual = ev(`TUTORIAL_FACTION_VOICE_F9V4A[${JSON.stringify(faction)}].tone`);
  if (actual !== tone) throw new Error(`Voice mismatch ${faction}: ${actual}`);
}

// Lore must remain indirect rather than stating hatred/disdain literally.
const allDialogue = ev("Object.values(TUTORIAL_DIALOGUE_PASS_F9V4A).flatMap(x=>Object.values(x).map(y=>y.text)).join('\\n')");
if (/\b(odio|odiano|odia|disprezza|disprezzano)\b/i.test(allDialogue)) throw new Error("Lore is too explicit");

// Overlay changes copy only: original message metadata survives.
ev("tutorialRuntimeState.scenarioId='lesson-1-exordium'");
context.__stepF9V4a = {id:'explain-defense',message:{speaker:'Narratore Exordium',portraitSet:'tutorial-exordium',expression:'explain',side:'right',text:'legacy'}};
const enriched = ev("tutorialRuntimeMessageForStepF9V4a(__stepF9V4a)");
if (enriched.speaker !== 'Narratore Exordium' || enriched.portraitSet !== 'tutorial-exordium' || enriched.expression !== 'explain' || enriched.side !== 'right') throw new Error(`Message metadata changed ${JSON.stringify(enriched)}`);
if (enriched.text === 'legacy') throw new Error("Dialogue overlay not applied");
context.__narrativeMessages = null;
context.narrativeOpen = (messages)=>{ context.__narrativeMessages = messages; return true; };
ev("tutorialRuntimeState.stepIndex=0; tutorialRuntimeState.scenario={steps:[__stepF9V4a]}; tutorialRuntimeState.step=__stepF9V4a");
...
console.log(JSON.stringify({
  ok:true,
  feature:"F9V4a Strategic Tutorial Content & Faction Voice Pass",
  terminalLockRegression:true,
  deckRecoveryRegression:true,
  dialogueScenarios:dialogueAudit.scenarios,
  dialogueOverrides:dialogueAudit.overrides,
  strategicTopics:dialogueAudit.topics,
  factionVoices:dialogueAudit.voices,
  indirectLore:true,
  scenarioStepContractUnchanged:true,
